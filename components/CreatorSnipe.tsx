...
const pill =
  "px-3 py-1 rounded-full text-xs font-medium border border-white/10 bg-white/5";

const WSOL =
  "So11111111111111111111111111111111111111112"; // input mint for SOL
const USDC =
...
function lamports(sol: number) {
  return Math.floor(clamp(sol, 0, 10_000) * 1e9);
}
...
function clampSortScore(rows: DexPair[]): DexPair[] {
  const scorePair = (r: DexPair): number => {
    ...
    const fdvScore =
      fdv <= 0 ? 0.6 : clamp(1 - Math.log10(Math.max(1, fdv)) / 7, 0, 1);
...
function dexsLink(chainId?: string, pairAddress?: string) {
  ...
  return `https://dexscreener.com/${encodeURIComponent(
    chainId
...
async function sendJupSwap({
  ...
  const signature = await walletSendTransaction(tx, connection, {
    skipPreflight: true,
    maxRetries: 2,
  });
...
// Token helpers
// ---------------------------------
async function getTokenBalanceLamports(
  conn: Connection,
  owner: PublicKey,
  mint: PublicKey
) {
...
async function fetchRugcheck(mint: string): Promise<RiskInfo | null> {
  try {
    ...
    const flags: string[] = [];
    if (data?.topHolders && data.topHolders[0]?.percentage > 30)
      flags.push("Top1 > 30%");
...
function heuristicFromDex(pair: DexPair): RiskInfo {
  ...
  if (typeof h1 === "number" && h1 > 60)
    notes.push("Parabolic 1h > 60% — возможен дамп");
...
function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setConnection(new Connection(ep, { commitment: "confirmed" }));
  }, []);
  ...
  useEffect(() => {
    if (!run) return;
    let p = Math.random() * 0.01 + 0.001;
    const id = setInterval(() => {
      p = Math.max(0.0001, p * (1 + (Math.random() - 0.5) * 0.2));
      setLive((old) => [...old.slice(-120), { t: Date.now(), p }]);
    }, 900);
  ...
  async function ensureRiskForMint(mint?: string, pair?: DexPair) {
    ...
    if (fromRug) {
      setRisk((m) => ({ ...m, [mint]: fromRug }));
      return;
    }
    if (pair) {
      setRisk((m) => ({ ...m, [mint]: heuristicFromDex(pair) }));
      return;
    }
    setRisk((m) => ({
      ...m,
      [mint]: { source: "unknown", ok: true, score: 60, notes: ["No external risk data"] },
    }));
  }
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      scanSolana,
      Math.max(5, Number(scanSec) || 30) * 1000
    );
    return () => clearInterval(id);
  }, [autoScan, scanSec, recentOnlyMins, minLiq, minVol, maxFdv, minH1, maxH1]);
  ...
  function pushTx(sig: string, note: string) {
    setTxs((xs) => [{ sig, t: Date.now(), note }, ...xs].slice(0, 20));
  }
  ...
  async function onBuyMint(outputMint: string, symbol?: string) {
    ...
    if (r && (!r.ok || r.score < 50)) {
      const sure = confirm(
        `Риск высокий (${r.source}, score ${r.score}). Всё равно купить?`
      );
      if (!sure) return;
    }
    try {
      const quote = await jupQuote({
        inputMint: WSOL,
        outputMint,
        amountLamports: lamports(stakeSol),
        slippageBps,
      });
      const { swapTransaction } = await jupSwapTx({
        quoteResponse: quote,
        userPublicKey: publicKey!.toBase58(),
      });
      const sig = await sendJupSwap({
        connection,
  ...
  async function onSellPctMint(inputMint: string, pct: number, symbol?: string) {
    ...
    if (!connected || !publicKey) return alert("Подключите кошелёк");
    try {
      const { baseAmount } = await getTokenBalanceLamports(
        connection,
        publicKey,
      ...
      const part = BigInt(
        Math.floor((Number(baseAmount) * clamp(pct, 0, 100)) / 100)
      );
  ...
  async function onSellMaxMint(inputMint: string, symbol?: string) {
    return onSellPctMint(inputMint, 100, symbol);
  }
    ...
    <div className="min-h-screen bg-[#0A0B0F] text-white">
      ...
      <section className="max-w-3xl mx-auto px-5 pt-12 pb-8 text-center">
        ...
        <h1 className="text-3xl md:text-4xl font-bold leading-tight">
          Полноценный снайпер: риск-чек + частичная продажа + USDC
        </h1>
        <p className="mt-3 text-white/70">
          Подключи кошелёк, выбери пару, покупай за SOL, продавай частями в SOL
          или USDC.
        </p>
        <div className="mt-4 flex items-center justify-center">
          <WalletMultiButton className="!bg-white/10 !border !border-white/10 !rounded-xl hover:!bg-white/20" />
        </div>
          ...
          >
            {run ? (
              <span className="inline-flex items-center gap-2">
                <Play className="w-4 h-4 text-emerald-400" /> Live
              </span>
            ) : (
              <span className="inline-flex items-center gap-2">
                <Pause className="w-4 h-4 text-yellow-300" /> Paused
              </span>
            )}
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 text-sm">
            ...
            <div className="text-xs flex items-end">
              <button
                ...
                className={`${pill} ${autoScan ? "border-emerald-400/50" : ""}`}
              >
        ...
        <div className={`${glass} rounded-2xl p-4`}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-xs opacity-70">
              послед. скан: {lastScanAt ? new Date(lastScanAt).toLocaleTimeString() : "—"}
            </div>
          </div>
                ...
                >
                  <div className="text-sm font-medium flex items-center justify-between">
                    ...
                    <div className="flex gap-2 text-xs">
                      <button
                        onClick={() => onBuyMint(mint, r.baseToken?.symbol)}
                        className="px-3 py-1 rounded-lg bg-gradient-to-r from-cyan-400 to-emerald-400 text-black font-semibold"
                        disabled={info ? !info.ok && info.score < 50 : false}
                      >
                        Buy
                      </button>
                      <div className="hidden sm:flex gap-1">
                        <button
                          onClick={() => onSellPctMint(mint, 25, r.baseToken?.symbol)}
                          className="px-2 py-1 rounded-lg bg-white/10 border border-white/10 hover:bg-white/15"
                        >
                          25%
                        </button>
                        <button
                          onClick={() => onSellPctMint(mint, 50, r.baseToken?.symbol)}
                          className="px-2 py-1 rounded-lg bg-white/10 border border-white/10 hover:bg-white/15"
                        >
                          50%
                        </button>
                        <button
                          onClick={() => onSellPctMint(mint, 75, r.baseToken?.symbol)}
                          className="px-2 py-1 rounded-lg bg-white/10 border border-white/10 hover:bg-white/15"
                        >
                          75%
                        </button>
                        <button
                          onClick={() => onSellPctMint(mint, 100, r.baseToken?.symbol)}
                          className="px-2 py-1 rounded-lg bg-white/10 border border-white/10 hover:bg-white/15"
                        >
                          Max
                        </button>
                  ...
                  <div className="text-xs opacity-80 flex flex-wrap gap-3 mt-1">
                    {r.priceUsd && (
                      <span className={pill}>${Number(r.priceUsd).toFixed(6)}</span>
                    )}
                    {typeof r.fdv === "number" && (
                      <span className={pill}>FDV ${Math.round(r.fdv).toLocaleString()}</span>
                    )}
                    {typeof r.liquidity?.usd === "number" && (
                      <span className={pill}>
                        Liq ${Math.round(r.liquidity.usd).toLocaleString()}
                      </span>
                    )}
                    {typeof r.volume?.h24 === "number" && (
                      <span className={pill}>
                        Vol24h ${Math.round(r.volume.h24).toLocaleString()}
                      </span>
                    )}
                    {typeof r.priceChange?.m5 === "number" && (
                      <span className={pill}>5m {pct(r.priceChange.m5)}</span>
                    )}
                    {typeof r.priceChange?.h1 === "number" && (
                      <span className={pill}>1h {pct(r.priceChange.h1)}</span>
                    )}
                    {r.pairCreatedAt && (
                      <span className={pill}>
                        new {Math.max(0, Math.floor((Date.now() - r.pairCreatedAt) / 60000))}m
                      </span>
                    )}
                    <RiskBadge
                      mint={mint}
                      info={info}
                      onLoad={() => ensureRiskForMint(mint, r)}
                    />
                  </div>
                </div>
              );
            })}
            {autoRows.length === 0 && autoScan && (
              <div className="text-xs opacity-70">
                Пока ничего не прошло по фильтрам.
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`${glass} rounded-2xl p-4`}>
          ...
          <div className="grid gap-2 text-xs">
            {txs.map((x) => (
              <a
                key={x.sig}
                href={`https://solscan.io/tx/${x.sig}`}
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-3 py-2 hover:border-cyan-400/40"
              >
                <span className="opacity-80">
                  {new Date(x.t).toLocaleTimeString()} · {x.note}
                </span>
          ...
          <div className="text-[11px] opacity-60 mt-2">
            Не является финсоветом. Риски — волатильность, ликвидность, MEV/слиппедж.
          </div>
        </div>
      </section>

      {/* FOOTER */}
      <footer className="max-w-3xl mx-auto px-5 py-10 text-xs opacity-60">
        Creator-Snipe x100 · Mainnet · Jupiter/WSOL/USDC • Session{" "}
        {short(Math.random().toString(36).slice(2, 10))}
      </footer>
    </div>
  );
}
  ...
  return (
    <label className="text-xs w-full">
      ...
      <input
        ...
        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 outline-none focus:border-cyan-400/50"
      />
    </label>
  );
}
...
function RiskBadge({
  ...
  const tone = info.ok && info.score >= 70
    ? "border-emerald-400/50"
    : info.ok
    ? "border-yellow-400/50"
    : "border-red-400/50";
  const title = `${info.source} • score ${info.score}${
    info.notes.length ? " • " + info.notes.join(", ") : ""
  }`;
  return (
    <span className={`${pill} ${tone}`} title={title}>
      Risk: {info.ok ? (info.score >= 70 ? "OK" : "Warn") : "Fail"}
    </span>
  );
}
